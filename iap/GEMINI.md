...
*   **`src/main.rs`**: Single entry point. 
    ...
    *   `iap_middleware`: Captures the `x-goog-iap-jwt-assertion` header and decodes identity context.
    *   `main`: Initializes the `StreamableHttpService`, sets up Axum with a `/health` route and IAP middleware.
...
*   `RUST_LOG`: Logging level (default: `info,iap_https_rust=debug`).
