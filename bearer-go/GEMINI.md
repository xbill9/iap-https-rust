...
- **Security:** Bearer Token authentication (via `MCP_BEARER_TOKEN`).
- **Tools:**
...
- **Key Commands:**
    ...
    - `make check`: Verifies server status and authentication mode.
    - `make info`: Runs system info report directly.
    - `make disk`: Runs disk usage report directly.
    - `make deploy`: Deploys to Google Cloud Run (service: `sysutils-bearer-go`).
