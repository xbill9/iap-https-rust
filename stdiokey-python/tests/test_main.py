...
class TestMain(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_check_api_key_status_with_arg_key_mismatch(self):
        with (
            ...
            mock_get_id.return_value = "test-project"
            mock_fetch.return_value = "expected-key"
            report, is_valid = await main.check_api_key_status(["--key", "wrong-key"])
            self.assertIn("Provided Key:     [FOUND]", report)
            self.assertIn("Cloud Match:      [MISMATCH]", report)
            self.assertFalse(is_valid)
            mock_fetch.assert_called_once_with("test-project")
    ...
    async def test_handle_call_tool_system_info(self):
        with patch("main.collect_system_info") as mock_collect:
            mock_collect.return_value = "mock system info"
            result = await main.handle_call_tool("local_system_info", {})
            self.assertEqual(result[0].text, "mock system info")
            mock_collect.assert_called_once()

    async def test_handle_call_tool_disk_usage(self):
        with patch("main.collect_disk_usage") as mock_collect:
            mock_collect.return_value = "mock disk usage"
            result = await main.handle_call_tool("disk_usage", {})
            self.assertEqual(result[0].text, "mock disk usage")
            mock_collect.assert_called_once()

    async def test_handle_call_tool_unknown(self):
        with self.assertRaises(ValueError):
            await main.handle_call_tool("unknown_tool", {})
