# Gemini Workspace for `bearer-python` (v0.5.0)
...
This document provides a developer-focused overview of the `bearer-python` (HTTP variant), tailored for use with Gemini.
...
*   **Security:** Google Cloud IAM (Bearer Token)
...
*   **Security Model**:
    *   The server is deployed with `--no-allow-unauthenticated`.
    *   Authentication is handled via **Google Cloud Identity Tokens**.
