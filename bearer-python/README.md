# bearer-python (v0.5.0)

`bearer-python` is a Model Context Protocol (MCP) server written in Python. It provides system utility tools to MCP clients (like Gemini) using the **Streaming HTTP transport**.
...
*   **Secure Deployment:** Protected by Google Cloud IAM (Bearer Token) when deployed.

## Security

This server is designed to be deployed securely. When deployed to Google Cloud Run via the provided `cloudbuild.yaml`, it uses `--no-allow-unauthenticated`.

Accessing the server requires a **Google Cloud Identity Token** passed in the `Authorization` header:
...
2.  **Security Configuration:**
    The deployment is configured with `--no-allow-unauthenticated`, meaning only authenticated callers with the correct IAM permissions (`roles/run.invoker`) can access the service.

3.  **Service Configuration:**
    The default service name is `sysutils-bearer-python` (as defined in `cloudbuild.yaml`). It deploys to `us-central1` by default.
